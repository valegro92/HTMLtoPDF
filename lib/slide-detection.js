...
(function () {
  ...
  function detectSlides() {
    ...
    // ── Strategy 1: Figli diretti del body, ≥3 e dimensioni viewport ──
    if (!slideElements) {
      var bodyChildren = getSizedChildren(document.body);
      if (bodyChildren.length >= 3 && areSimilarSize(bodyChildren)) {
    ...
    if (!slideElements) {
      ...
      for (var i = 0; i < bodyChildrenAll.length; i++) {
        ...
        if (wrapperChildren.length >= 3 && areSimilarSize(wrapperChildren)) {
    ...
    // ── Strategy 3: Deep search ≥3 figli simili viewport-size, max profondità 3 ──
    if (!slideElements) {
      ...
      for (var j = 0; j < allContainers.length; j++) {
        ...
        if (children.length >= 3 && areSimilarSize(children)) {
